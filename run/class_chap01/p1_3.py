...
def p1_3(self):
	...
	for idx, ans in enumerate(answersSi):
		try:
			...
			print( f"Si: when Tk = {int(Tk[idx])}K, intrinsic concentration ni = {ni_Si[idx]:.3e}/cm^3" )
	...
	for idx, ans in enumerate(answersGe):
		try:
			...
			print( f"Si: when Tk = {int(Tk[idx])}K, intrinsic concentration ni = {ni_Ge[idx]:.3e}/cm^3" )
		except AssertionError as e:
			print( f"AssertionError {pnum}: {e}" )
