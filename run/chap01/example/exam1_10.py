...
def exam1_10(self) -> None:
	...
	# ----------------------------------------------------------------------------
	VPS_iter_val:List[float] = [round(0 + i * 0.01, 2) for i in range(501)]  # 501 values from 0 to 5
	...
	# Diode current - create first y-axis (id1) on to-be-shared x-axis (VPS)
	ax1.plot( VPS_x_coord, id1, color='b', label='id1' )  # customize color, recall 'k' = black
	ax1.set_ylabel('id1 A', color='b')  # Set y-axis label for the first axis
	ax1.tick_params(axis='y', labelcolor='b')
	plt.legend()  # adds a legend to label the current curve

	# Diode voltage (vd1) - create second y-axis (vd1) sharing the same x-axis (VPS)
	ax2 = ax1.twinx()
	ax2.plot( VPS_x_coord, vd1, color='r', label='vd1' )
	ax2.set_ylabel('vd1', color='r')  # Set y-axis label for the second axis
